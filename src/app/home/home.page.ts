...
export class HomePage implements OnInit {
    ...
    addDriver(email, id) {
        ...
        const imagenRef = firebase.database().ref('pedidos-aceptados').
        ...
        this.firestoreService.insertarControl('pedido-tracking',
            firebase.auth().currentUser.email, {id, email}).then(() => {
    ...
    getLocation() {
        ...
        this.watch = this.geolocation.watchPosition().subscribe((data) => {
            ...
            this.fixLat = parseFloat('coords' in data ? data.coords.latitude.toFixed(3) : '0');
            this.fixLng = parseFloat('coords' in data ? data.coords.longitude.toFixed(3) : '0');
    ...
    sendLocationRealTime() {
        // this.count+=1
        const imagenRef = firebase.database().ref('Ubicaciones_choferes').
